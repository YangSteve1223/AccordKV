...
\textbf{Structure-Dependent Advantage.} Unlike global SVD which enforces a uniform rank $r$ across all tokens, cluster-conditional SVD assigns per-cluster ranks $\{r_c^*\}$. Empirically, on semantically clustered data (k=8), this achieves 11.6--12.2× lower reconstruction error than global SVD (Table~\ref{tab:method_d}). The advantage is data-dependent: when clusters exhibit heterogeneous rank requirements (i.e., $\max_c r_c^* \ll r$), the per-cluster approach concentrates rank budget where singular values decay slowly. Formally, let $r = \max_c r_c^*$. Then for any cluster $c$:
\[
\sigma_{r_c^*+1}^{(c)} \leq \sigma_{r+1}^{(c)},
\]
with strict inequality when $r_c^* < r$ and cluster $c$ has low intrinsic rank. The observed 11.6--12.2× improvement on clustered workloads in Table~\ref{tab:method_d} validates that the semantic clustering step creates clusters where $r_c^* \ll r$, enabling significant rank savings.
...
\textbf{Structure-Dependent Advantage.} Unlike global SVD with a uniform rank, cluster-conditional SVD assigns per-cluster ranks $\{r_j^*\}$. The advantage is structure-dependent: when clusters exhibit low intrinsic rank ($r_j^* \ll r$), per-cluster truncation achieves:
\[
\sigma_{r_j^*+1}^{(j)} \leq \sigma_{r+1}^{(j)} \quad \forall j,
\]
with strict inequality for clusters where $r_j^* < r$. The 11.6--12.2× improvement on clustered workloads (Table~\ref{tab:method_d}) confirms that semantic clustering creates clusters with $r_j^* \ll r$, enabling substantial rank savings.
