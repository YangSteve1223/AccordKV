...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            height=0.6\textwidth,
            xlabel={Rank},
            ylabel={Relative Error ($rel_{err}$)},
            legend style={at={(0.5,-0.2)}, anchor=north, legend columns=-1},
            grid=major,
            xmode=log,
            log basis x={2},
            xtick={4,16,64,128,256},
            ymin=0, ymax=1,
            mark size=2pt
        ]
        \addplot[blue, mark=o, solid, line width=1.5pt] coordinates {
            (4, 0.9001) (16, 0.8878) (64, 0.8765) (128, 0.8749) (256, 0.8749)
        };
        \addplot[blue, mark=square, dashed, line width=1.5pt] coordinates {
            (4, 0.8978) (16, 0.8659) (64, 0.8147) (128, 0.7957) (256, 0.7957)
        };
        \addplot[red, mark=o, solid, line width=1.5pt] coordinates {
            (4, 0.8959) (16, 0.8784) (64, 0.8547) (128, 0.8440) (256, 0.8390)
        };
        \addplot[red, mark=square, dashed, line width=1.5pt] coordinates {
            (4, 0.9369) (16, 0.9077) (64, 0.8551) (128, 0.8264) (256, 0.8105)
        };
        \legend{Mistral K, Mistral V, Gemma K, Gemma V}
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \addplot[blue, mark=o, solid, line width=1.5pt] coordinates {
            (8, 0.9413) (32, 0.9831) (64, 0.9954)
        };
        \addplot[blue, mark=square, dashed, line width=1.5pt] coordinates {
            (8, 0.5995) (32, 0.8032) (64, 0.9178)
        };
        \addplot[red, mark=triangle, solid, line width=1.5pt] coordinates {
            (8, 0.8449) (32, 0.9236) (64, 0.9597)
        };
        \addplot[red, mark=diamond, dashed, line width=1.5pt] coordinates {
            (8, 0.5317) (32, 0.7173) (64, 0.8325)
        };
        \legend{Mistral K, Mistral V, Gemma K, Gemma V}
        \end{axis}
    \end{tikzpicture}
...
\begin{table}[t]
    \centering
    \caption{Downstream perplexity (PPL) on WikiText-2-derived long-context sequences. PPL measured on tokens 1--256; lower is better. $\Delta$ = relative degradation vs. FP16 baseline.}
    \begin{tabular}{l|c|c|c}
    \toprule
    \textbf{Configuration} & \textbf{Rank} & \textbf{Avg PPL} & $\Delta$ \\
    \midrule
    Mistral FP16 (baseline) & —       & TBD              & —        \\
    Mistral FP16 + SVD      & 8       & TBD              & TBD      \\
    Mistral FP16 + SVD      & 16      & TBD              & TBD      \\
    Mistral FP16 + SVD      & 32      & TBD              & TBD      \\
    Mistral INT4 + SVD      & 8       & TBD              & TBD      \\
    \bottomrule
    \end{tabular}
    \label{tab:ppl_results}
\end{table}

\textbf{Expected trend}: Higher rank reduces PPL degradation. INT4 quantization at rank 8 introduces additional error beyond FP16 SVD. The Value bottleneck analysis (Section~\ref{sec:kvasym}) predicts that even rank-32 SVD cannot fully recover V information (cumulative variance 0.8032 vs. K 0.9831).
