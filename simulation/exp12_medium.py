#!/usr/bin/env python3
"""Medium grid for exp12 - balanced coverage and speed."""
...
def run_medium():
    ...
    # Grid: 4 types × 2 lens × 2 ds × 4 levels × 3 wavelets × 2 seeds
    # = 384 configs (but levels limited by max valid)
    configs = []
            ...
            for d in [128]:
                for level in [1, 2, 3, 4]:
                    for wavelet in ['db4', 'haar', 'sym4']:
                        for seed in [0, 1]:
                            configs.append((kt, kl, d, level, wavelet, seed))
    
    print(f"Running {len(configs)} configs...")
    
    for i, (kt, kl, d, level, wavelet, seed) in enumerate(configs):
        make_kv = make_kv_factory(kt)
        K, V = make_kv(kl, d, seed=seed)
        Q = (np.random.default_rng(seed+1000).standard_normal((16, d)) * 0.5).astype(np.float32)
